...
uint32_t maxSizeProcessed = 0;
uint32_t maxSizeProcessedA = 0;
uint32_t maxSizeProcessedB = 0;
...
void *tmerge_recurse(void *params) {
    ...
    // Mapping
    if (MAP) printf("call: size %u    start %u    depth %u    maxproc %u   a: %u  b: %u\n", size, start, depth, maxSizeProcessed, maxSizeProcessedA, maxSizeProcessedB); 
    ...
    if (size > 2) {
        ...
        */
        ...
        thpool_add_work(tp, (void*)tmerge_recurse, (void*)&args_a);
        
        // pthread_join(tId, NULL);
        // if (size > maxSizeProcessedA) { 
        //     pthread_join(tId_a, NULL);
        //     maxSizeProcessedA = size;
        // }
        ...
        thpool_add_work(tp, (void*)tmerge_recurse, (void*)&args_b);

        // if (size > maxSizeProcessedB) { 
        //     pthread_join(tId_b, NULL);
        //     maxSizeProcessedB = size;
        // }
        // pthread_join(tId, NULL);
        // if (size > maxSizeProcessed) pthread_join(tId_b, NULL);

        // if (size > maxSizeProcessed) {
        //     pthread_join(tId_a, NULL);
        //     pthread_join(tId_b, NULL);
        // }
...
// void sort_tmerge(uint32_t *num, uint32_t size, uint32_t *random)  //DEBUG
{
    ...
    thpool_add_work(tp, (void*)tmerge_recurse, (void*)&args);
    thpool_wait(tp);
    thpool_destroy(tp);
