 ...
 * @version 0.11
 * Fixed incorrect behaviour with some buffer sizes
 * Updated test to work with a range of buffer sizes 
 */
...
void cbuffer_write(uint8_t value) {
    ...
    if (count < CBUFFER_SIZE) {
        tail = (tail + CBUFFER_SIZE - 1) % CBUFFER_SIZE;
        count++;
    }
    else
        head = (head + CBUFFER_SIZE - 1) % CBUFFER_SIZE;
}
